...
export const Login: React.FC = () => {
  ...
  const { addStudent, students, departments, classes } = useStudents();
  ...
  useEffect(() => {
    const firstClassForDepartment = classes.find(cls => cls.departmentName === selectedDept) ?? classes[0];
    if ((!selectedClass || !classes.some(cls => cls.code === selectedClass)) && firstClassForDepartment) {
      setSelectedClass(firstClassForDepartment.code);
    }
  }, [classes, selectedClass, selectedDept]);
            ...
            {role === 'student' && (
              <>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  >
                    {classes
                      .filter((c) => !selectedDept || c.departmentName === selectedDept)
                      .map((c) => (
