  ...
  const menuItems =
    currentUser?.role === "admin"
      ? [
          ...
          { id: "classes", label: "Quản lý", icon: School },
        ]
      : [{ id: "profile", label: "Hồ sơ của tôi", icon: UserCheck }];

  return (
    <aside
      className={`h-screen fixed left-0 top-0 bg-bg-secondary border-r border-border-color flex flex-col p-4 z-[100] transition-all duration-300 max-[576px]:hidden ${
    ...
    >
      ...
      <div className="flex items-center gap-3 pb-6 mb-6 border-b border-border-color">
        ...
        <span
          className={`font-title text-xl font-extrabold bg-accent-gradient bg-clip-text text-transparent whitespace-nowrap transition-opacity duration-300 ${
            isCollapsed
      ...
      <nav className="flex-1">
        <ul className="flex flex-col gap-2">
          {menuItems.map((item) => {
            ...
            const isActive =
              currentUser?.role === "admin" ? activeTab === item.id : true;
            return (
              <li key={item.id} className="w-full">
                <button
                  onClick={() =>
                    currentUser?.role === "admin" && setActiveTab(item.id)
                  }
      ...
      {/* Footer */}
      <div
        className={`pt-4 border-t border-border-color flex items-center gap-2 ${
